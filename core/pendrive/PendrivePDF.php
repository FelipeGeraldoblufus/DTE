 ...
 */
class PendrivePDF extends PDF_Pendrive
{
    ...
    private function agregarEmisor(array $emisor, $x = 10, $y = 20, $w = 120, $w_img = 50, $font_size = null, array $color = null)
    {
        // logo del documento
        if (isset($this->logo)) {
            $this->Image(
        ...
        $this->MultiTexto(!empty($emisor['RznSoc']) ? $emisor['RznSoc'] : $emisor['RznSocEmisor'], $x-9, $this->y+15, 'L', $w);
        $this->setFont('', '', $font_size ? $font_size : 9);
        $this->SetTextColorArray([0,0,0]);
        $this->MultiTexto(!empty($emisor['GiroEmis']) ? $emisor['GiroEmis'] : $emisor['GiroEmisor'], $x-9, $this->y, 'L', $w);
        $ciudad = !empty($emisor['CiudadOrigen']) ? $emisor['CiudadOrigen'] : Chile::getCiudad($emisor['CmnaOrigen']);
        $this->MultiTexto($emisor['DirOrigen'].', '.$emisor['CmnaOrigen'].($ciudad?(', '.$ciudad):''), $x-9, $this->y, 'L', $w);
        if (!empty($emisor['Sucursal'])) {
            $this->MultiTexto('Sucursal: '.$emisor['Sucursal'], $x-9, $this->y, 'L', $w);
        }
        $contacto = [];
