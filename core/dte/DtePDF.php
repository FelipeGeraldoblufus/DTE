...
class DtePDF extends PDF_Dte
{
    ...
    private $referencia_cols = [
        'NroLinRef' => ['title'=>'Ref.', 'align'=>'left', 'width'=>10],
        #'RazonRef' => ['title'=>'Razón Referencia', 'align'=>'left', 'width'=>0],
        'TpoDocRef' => ['title'=>'Doc. Referencia', 'align'=>'left', 'width'=>70],
        'FolioRef' => ['title'=>'Folio', 'align'=>'right', 'width'=>30],
        'FchRef' => ['title'=>'Fecha', 'align'=>'right', 'width'=>20],
    ...
    private $detalle_fuente = 7; ///< Tamaño de la fuente para el detalle en hoja carta
    ...
    private function agregarEmisor(array $emisor, $x = 10, $y = 20, $w = 120, $w_img = 60, $font_size = null, array $color = null)
{
    ...
    private function agregarFolio($rut, $tipo, $folio, $sucursal_sii = null, $x = 130, $y = 20, $w = 70, $font_size = null, array $color = null)
    {
        ...
        $this->setFont ('', 'B', $font_size ? $font_size : 11);
    ...
    private function agregarDatosEmision($IdDoc, $CdgVendedor, $x = 10, $offset = 22, $mostrar_dia = true)
    {
        ...
        if ($x==10) {
            ...
            if (!empty($IdDoc['FmaPago'])) {
                $this->MultiTexto('Venta: '.$this->formas_pago[$IdDoc['FmaPago']], $x, $y, 'R');
            }
    ...
    private function agregarReceptor(array $Encabezado, $x = 10, $offset = 22, $w = 190, $font_size = null)
    {
        $y = $this->GetY();
    ...
    $this->Rect($x, 81.5, 130, 124, 'L', ['all' => ['width' => 0.3, 'color' => [3,3,3]]]);
        ...
        foreach ($detalle as &$item) {
            ...
            foreach ($item as $col => $valor) {
                if ($col=='DscItem' and !empty($item['DscItem'])) {
                    $item['NmbItem'] .= !$this->item_detalle_posicion ? (isset($html)?'<br/>':"\n") : ': ';
                    if (isset($html)) {
                        $item['NmbItem'] .= '<span style="font-size:0.7em">'.$item['DscItem'].'</span>';
                    } else {
                        $item['NmbItem'] .= $item['DscItem'].'<br>';
                    }
                }
    ...
    private function agregarAcuseRecibo($x = 145, $y = 209, $w = 55, $h = 32)
    {
        $this->SetTextColorArray([0,0,0]);
        $this->setFont('', 'B', 6);
        $this->Texto('Nombre:', $x, $y+1);
        $this->Texto('_____________________________________', $x + 9);
        $this->Texto('RUT:', $x, $this->y+5);
        $this->Texto('_______________', $x+6);
        $this->Texto('Firma:', $x+25, $this->y);
        $this->Texto('__________________', $x+32);
        $this->Texto('Fecha:', $x, $this->y+6);
        $this->Texto('____________', $x+9);
        $this->Texto('Recinto:', $x+23, $this->y);
        $this->Texto('__________________', $x+32);
        $this->setFont('', 'B', 6);
        ...
        $this->Rect($x, $y-1, $w, $h, 'D', ['all' => ['width' => 0.3, 'color' => [0, 0, 0]]]);
    }
    ...
    private function agregarTimbre($timbre, $x_timbre = 10, $x = 10, $y = 208, $w = 70, $font_size = 8)
    {
        $this->setYbaja($y);
        if ($timbre!==null) {
            $style = [
                ...
                'hpadding' => 0,
                'vpadding' => 0,
                'module_width' => 6, // width of a single module in points
            ...
            $this->write2DBarcode($timbre, 'PDF417,,'.$ecl, $x_timbre, $y, $w, 0, $style, 'N');
    ...
    private function agregarSubTotal(array $detalle, $x = 81, $offset = 25)
    {
        $y = $this->getYbaja();
        $this->setYbaja($y);
        $this->setFont('', 'B', 9);
    ...
    private function agregarTotales(array $totales, $x = 81, $offset = 25)
    {
        $y = $this->getYbaja();

        $this->Rect($x, $y, 59, 32, 'D', ['all' => ['width' => 0.3, 'color' => [0,0,0]]]);
        // normalizar totales
        $totales = array_merge([
            'TpoMoneda' => false,
            'MntNeto' => false,
        ...
        $glosas = [
            ...
            'MntNeto' => 'Neto $',
            'MntExe' => 'Exento $',
            'IVA' => 'IVA ('.str_replace('.00', '',$totales['TasaIVA']).'%) $',
