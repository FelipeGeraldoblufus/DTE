...
class Cesion
{
    ...
    {
        $this->datos['Cesion']['DocumentoCesion']['Cedente'] = Arreglo::mergeRecursiveDistinct([
    ...
    {
        $xml_unsigned = (new XML())->generate($this->datos)->saveXML();
        $xml = $Firma->signXML($xml_unsigned, '#Cesion', 'DocumentoCesion');
        if (!$xml) {
            Log::write(
                Estado::DTE_ERROR_FIRMA,
                Estado::get(Estado::DTE_ERROR_FIRMA, '#Cesion')
            );
            return false;
        }
        $this->xml = new XML();
