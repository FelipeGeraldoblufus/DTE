...
class Aec extends Envio
{
    ...
    public function generar()
    {
        ...
        $this->xml_data = $this->Firma->signXML($xmlEnvio, '#AEC', 'DocumentoAEC', true);
    ...
    public function enviar()
    {
        ...
        if (!$this->schemaValidate())
            return false;
        // solicitar token
        $token = Sii\Autenticacion::getToken($this->Firma);
        if (!$token)
            return false;
        // enviar AEC
    ...
    private function enviarRTC($email, $empresa, $dte, $token, $retry = null)
    {
        ...
        $url = 'https://'.Sii::getServidor().'.sii.cl/cgi_rtc/RTC/RTCAnotEnvio.cgi';
