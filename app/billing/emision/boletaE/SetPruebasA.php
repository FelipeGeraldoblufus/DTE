...
$montoNeto1 = round($totalAmount1 / 1.19);
$montoIVA1 = $totalAmount1 - $montoNeto1;
...
$montoNeto2 = round($baseAmount2 / 1.19);
$montoIVA2 = $baseAmount2 - $montoNeto2;
...
$montoNeto3 = round($totalAmount3 / 1.19);
$montoIVA3 = $totalAmount3 - $montoNeto3;
...
// Calcular neto e IVA solo del monto afecto
$montoNeto4 = round($baseAmount4_1 / 1.19);  // 10,689
$montoIVA4 = $baseAmount4_1 - $montoNeto4;   // 2,031
...
$baseAmount5 = 700 * 5; // 3,500
$montoNeto5 = round($baseAmount5 / 1.19); // Net amount
$montoIVA5 = $baseAmount5 - $montoNeto5; // Tax amount
...
// Crear EnvioBoleta y procesar DTEs
...
// Después de generar y firmar los DTEs, antes del EnvioBoleta
$totalMontos = array_sum(array_map(function($dte) {
    return $dte->getDatos()['Encabezado']['Totales']['MntTotal'];
}, $allDTEs));

// Después de generar los DTEs
$ConsumoFolios = new ConsumoFolio();
$ConsumoFolios->setFirma($Firma);
...
$ConsumoFolios->setCaratula($caratulaRCOF);
...
foreach ($allDTEs as $DTE) {
    ...
    // Obtener montos
    $montoExento = isset($totales['MntExe']) ? $totales['MntExe'] : 0;
    $montoTotal = $totales['MntTotal'];
    
    // Restar el exento antes de calcular neto e IVA
    $montoAfecto = $montoTotal - $montoExento;
    
    // Calcular neto e IVA sobre el monto afecto
    $montoNeto = round($montoAfecto / 1.19);
    $montoIVA = $montoAfecto - $montoNeto;
    
    $ConsumoFolios->agregar([
        'TpoDoc' => 39,
        'FchDoc' => date('Y-m-d'),
        'NroDoc' => $DTE->getFolio(),
        'TasaImp' => 19,
        'MntNeto' => $montoNeto,      // Este será 43831
        'MntIva' => $montoIVA,        // Este será 8329
        'MntExe' => $montoExento,     // 2000
        'MntTotal' => $montoTotal     // 54160
    ]);
}

$rcofXML = $ConsumoFolios->generar();
$archivoRCOF = $carpeta . 'RCOF_' . date('Ymd') . '.xml';
file_put_contents($archivoRCOF, $rcofXML);

// Enviar RCOF al SII (descomentar cuando esté listo para enviar)
/*
try {
    $track_id = $ConsumoFolios->enviar();
    if ($track_id) {
        echo "RCOF enviado correctamente. TrackID: " . $track_id;
    } else {
        echo "Error al enviar RCOF";
    }
} catch (Exception $e) {
    echo "Error al enviar RCOF: " . $e->getMessage();
}
// Obtener token y preparar envíos
...
echo "EnvioBoleta - Código HTTP: $httpCode\n";
echo "EnvioBoleta - Respuesta: $response\n";
