...
if (isset($_POST['data']) && !empty($_POST['data'])) {
  switch ($_POST['data']) {
    ...
    if (!empty($_POST['rut'])) {
    	...
    	$path  = $Directorio->creaDirectorio('../../../client/fotos/empleados/' . $rut);
    	if ($_FILES['foto']['name'][0] === '' || $_FILES['foto']['tmp_name'][0] === '') {
  		    $foto = 'client/fotos/empleados/default.png';
    	} else {
    		$files = array();
    ...
    if ($_FILES['foto']['name'][0] === '' || $_FILES['foto']['tmp_name'][0] === '') {
    	...
    	$Directorio = new Directorio();
    	$path  = $Directorio->creaDirectorio('../../../client/fotos/empleados/'.$_POST['id']);
    	$files = array();
    	foreach ($_FILES['foto'] as $k => $l) {
    		foreach ($l as $i => $v) {
    			if (!array_key_exists($i, $files)) {
    ...
    $Empleado->updEmpleado($rut, $datos);
