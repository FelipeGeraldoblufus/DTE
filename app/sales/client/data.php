...
if (isset($_POST['data']) && !empty($_POST['data'])) {
	switch ($_POST['data']) {
		...
		if (!empty($_POST['rut'])) {
			...
			$datos = [
				...
				!empty($_POST['correo_envio']) ? $_POST['correo_envio'] : '',
		...
		if (!empty($_POST['rut_edit'])) {
			...
			$datos = [
				...
				!empty($_POST['correo_envioedit']) ? $_POST['correo_envioedit'] : '',
    ...
    $Cliente->delCliente($id);
    break;
	}
}

if (isset($_GET['id']) && !empty($_GET['id'])) {
  $datos = $Cliente->getCliente($_GET['id']);
