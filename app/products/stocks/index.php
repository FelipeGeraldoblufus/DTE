...
$listaBodega = $Bodega->listaBodega();
...
<html lang="es">
<head>
  <title>DTE Chile | Stock y Bodegas</title>
  <!-- BEGIN META -->
  <meta charset="utf-8">
    ...
    <div id="content">
      <section>
        ...
        <div class="section-header">
          <ol class="breadcrumb">
            <li><a href="<?php echo BASEURL.'app/products/' ?>">Productos</a></li>
        ...
        <div class="section-body">
          <div class="col-lg-offset-1 col-md-10 col-sm-12">
            ...
            <div class="card">
              <div class="card-body">
                <?php if (isset($listaStock) && !empty($listaStock)): ?>
                <table id="dttStock" class="table table-hover">
                  <thead>
                    <tr>
                      <th>Producto</th>
                      <th>Stock</th>
                  ...
                  <tbody>
                    <?php foreach ($listaStock as $stock): ?>
                      <tr>
                        <td><?php echo $stock['producto_codigo'] ?></td>
                        <td><?php echo $stock['stock'] ?></td>
                        <td><?php echo $Bodega->nameBodega($stock['bodega_id']); ?></td>
                        <td class="text-right">
                          <a class="btn btn-icon-toggle update" href="javascript:void(0)" id="<?php echo $stock['id'] ?>" data-toggle="tooltip" data-placement="top" data-original-title="Editar"><i class="fa fa-pencil"></i></a>
                          <a class="btn btn-icon-toggle delete" href="javascript:void(0)" id="<?php echo $stock['id'] ?>" data-toggle="tooltip" data-placement="top" data-original-title="Eliminar"><i class="fa fa-trash-o"></i></a>  
                        </td>
                      </tr>
                    <?php endforeach ?>
                  </tbody>
          ...
          <form autocomplete="off" class="sendForm">
            <div class="form-group">
              ...
              <select id="" name="producto" class="form-control">
                <?php foreach ($codigoProducto as $producto): ?>
                  <option value="<?php echo $producto['codigo'] ?>"><?php echo  $producto['codigo'].' - '.$producto['nombre'] ?></option>
                <?php endforeach ?>
              </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <select id="bodega" name="bodega" class="form-control">
                <?php foreach ($listaBodega as $bodega): ?>
                  <option value="<?php echo $bodega['id'] ?>"><?php echo $bodega['bodega'] ?></option>
                <?php endforeach ?>
              </select>
            </div>
          </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form autocomplete="off" class="sendForm">
            <div class="form-group">
              <label id="">Producto</label>
              <select id="" name="producto_edit" class="form-control">
                <?php foreach ($codigoProducto as $producto): ?>
                  <option value="<?php echo $producto['codigo'] ?>"><?php echo  $producto['codigo'].' - '.$producto['nombre'] ?></option>
                <?php endforeach ?>
              </select>
            ...
            <div class="form-group">
              <label for="">Stock</label>
              <input type="number" name="stock_edit" id="stock" class="form-control">
            </div>
            <div class="form-group">
              <label id="">Bodega</label>
              <select id="bodega" name="bodega_edit" class="form-control">
                <?php foreach ($listaBodega as $bodega): ?>
                  <option value="<?php echo $bodega['id'] ?>"><?php echo $bodega['bodega'] ?></option>
                <?php endforeach ?>
              </select>
    ...
    $(document).on('click', '.update', function(){
      ...
      $.ajax({
        ...
        success:function(data)
        {
          ...
          $('[name="bodega_edit"]').val(data.bodega_id);
          $('[name="producto_edit"]').val(data.producto_codigo);
        }
      })
    });
    ...
    $(document).on('click', '.delete', function(){
      ...
      $.ajax({
        ...
        success:function(data)
        {
          $('#delModal').modal('show');
          $('[name="id"]').val(id);
          $('#valor').html(data.stock+' del '+data.producto_codigo);
        }
      })
    });
  </script>
